...
namespace Maternity
{
    public class MaternityPatientDetailCardWaitingForPatientState : MaternityPatientDetailCardBaseState
    {

        private MaternityWaitingRoomBed bed;

        public MaternityPatientDetailCardWaitingForPatientState(MaternityWaitingRoomBed bed, MaternityPatientDetailCard card) : base(card)
        {
            this.bed = bed;
        }
        ...
        private void Bed_OnStateChanged()
        {
            switch (bed.StateManager.State.GetTag())
            {
                case MaternityWaitingRoomBed.State.OR:
                    card.StateManager.State = new MaternityPatientDetailCardPatientInBedState(bed, bed.GetPatient(), card);
                    break;
            }
        }
