...
namespace Hospital.Connectors
{
    public static class UserConnector
    {
        ...
        public static Task SaveAsync(UserModel user)
        {
            return Context.SaveAsync<UserModel>(user);
        }

        public static Task<UserModel> LoadAsync(string hash)
        {
            return Context.LoadAsync<UserModel>(hash);
        }
    }
}